...
async function addNewDentist (dentist: Dentist, map: leaflet.Map): Promise<void> {
  geoCodeAddress('Kakelösagatan 61 43144 Mölndal').then((result: Place) => {
    console.log(result)
    const pictureAndName = `<div class="flex flex-wrap items-center gap-4"><div class="picture"><img class="dentistProfileMap" src="${dentist.picture}"></div><div class="details"><p>${dentist.name.firstName} ${dentist.name.lastName}</p><p>${dentist.address.street} ${dentist.address.houseNumber}</p><p>${dentist.address.zip} ${dentist.address.city}</p><button class="log-in-btn h-12 mb-6 bg-secondary rounded-xl text-base">See slots</button></div>  </div>`
