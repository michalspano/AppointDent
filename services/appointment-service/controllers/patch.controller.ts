...
const bookAppointment = async (req: Request, res: Response): AsyncResObj => {
  ...
  const patientMessage = toBook ? `Your booking on ${utils.formatDateTime(appointment.start_timestamp)} was confirmed.` : `Your booking on ${utils.formatDateTime(appointment.start_timestamp)} was canceled.`;
